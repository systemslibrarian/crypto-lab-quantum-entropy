 ...
 *    an unknown backdrop, or a `color-mix()`. This page has two of them and
 *    both are load-bearing: the Toeplitz entropy-budget bar fills its available
 *    band with `color-mix(in oklab, var(--accent) 45%, transparent)` and its
 *    overdraft band with `color-mix(in oklab, #dc2626 55%, transparent)`, and
 *    the `k = …` / `m = …` / `margin` / `overdraft` labels sit directly on
 *    those. That figure is the whole point of panel 4 — it is where "you cannot
 *    extract more entropy than you have" is made visible — and axe files every
 *    one of those labels under `incomplete`.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Nothing on this page uses
 *    `opacity` today, which is a fact about the current stylesheet rather than
 *    about the page, and it is exactly the kind of fact a de-emphasised
 *    "pending" state introduces without anyone re-measuring. The walk models it
 *    regardless.
 *
 ...
 *    box is `.bitscroll` — `max-height: 7.5rem; overflow: auto` — and there are
 *    three of them, holding the raw stream, the debiased output and the
 *    extracted output. All three overflow on arrival: the raw stream alone
 *    renders 1024 bits as space-grouped text. Content scrolled past that cap is
 *    not dimmed or partly drawn; it is absent from the frame, its rect falls
 *    outside every ancestor's box, and a walk that measured it would find
 *    nothing behind it and fall through to WHITE — inventing a failure for
 *    every bit below the fold of every stream on the page.
 *
 ...
 *    branch is LIVE here: the lab draws three SVG figures — the beam-splitter
 *    diagram, the Shannon-vs-min-entropy chart, and the entropy-budget bar —
 *    and in the budget bar the `margin`/`overdraft`/`k`/`m` labels are painted
 *    over `<rect>` siblings drawn earlier in the same `<svg>`. An ancestor-only
 *    walk would composite them onto the `<svg>`'s own transparent background
 *    and report a ratio nothing on screen has.
 *
 ...
 * rule. `nontext.ts` measures those separately. This page's only pseudo-element
 * with a glyph is the UA's own `<summary>` disclosure triangle on the four
 * `<details>`; the stylesheet does not suppress it and does not replace it.
 *
 * SECOND, the SVG STROKES that are not text: the two entropy curves
 * (`--ok-ink` Shannon, `--danger-ink` min-entropy), the beam paths, the
 * `--chart-grid` axes and the Toeplitz matrix cells. Those are WCAG 1.4.11
 * graphical objects rather than 1.4.3 text, no oracle in this repo reaches
 * them, and they are hand-measured with the numbers recorded in the commit.
 */
 ...
 * there is nothing short-lived left: `vnPanel.ts`'s "Run remaining pairs"
 * checks `matchMedia('(prefers-reduced-motion: reduce)')` and jumps the strip
 * straight to its end state instead of stepping it on a 140ms interval, so
 * `.vn-pair.current` — the class that lives for one tick during playback — is
 * reachable only through the "Step one pair" button, which the drive presses.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives 140ms is shorter than a full axe
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * that is the three `.bitscroll` regions (`overflow: auto` under a 7.5rem
     * cap), each of which overflows as soon as it holds a stream. `.sr-only` is
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. Both branches are LIVE on this page. The FILLED branch
      // matters in the entropy-budget bar, where the `k`, `m`, `margin` and
      // `overdraft` labels are painted over <rect>s drawn earlier in the same
      // <svg>. The stroke-only guard matters in the beam-splitter diagram and
      // the entropy chart, which between them draw ten <line>s and two <path>s
      // that carry `stroke` and leave `fill` at its initial black — and the
      // chart's dashed 1.0 gridline runs straight under the "Shannon H" label.
     ...
     *
     * What this lab hides is a closed set of six elements, enumerated by
     * grepping `aria-hidden` across `index.html` and `src/`, and every member of
     * it is decorative: the four `→` `.arch-arrow` glyphs between the stages of
     * the pipeline diagram, and the shared header's two glyph-free SVG marks.
     * Each arrow sits between two `.arch-stage` boxes whose text names the
     * stage, in `--text-dim` on the same panel surface as the prose beside it,
     * so measuring `.note` measures the arrow's ink too.
     *
     * Nothing on this page hides a VALUE. No bit stream, no entropy figure, no
     * cutoff, no verdict box and no health-test result is inside an
     ...
     *
     * This lab draws three SVG figures and builds two of them by assigning
     * `innerHTML` on the `<svg>` from a template literal, which is exactly how
     * the mistake usually arrives: interpolating a string ARRAY into one makes
     * JS join it with commas, leaving a run of "," text nodes inside whatever
     * element wraps them — and inside a `<g>` or directly under an `<svg>`
