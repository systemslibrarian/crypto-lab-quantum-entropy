...
4. **Toeplitz extraction** — a real m×256 Toeplitz matrix from a fresh CSPRNG seed, real GF(2) matrix–vector multiply, with the Leftover Hash Lemma receipt: ε ≤ ½·√(2^(m−k)) and a live entropy-budget bar (k bits available of 256 raw, m demanded, margin or overdraft shaded). Crank m past k and watch the security margin evaporate — the output still *looks* perfectly random, which is exactly why the accounting, not the appearance, is the verdict.
